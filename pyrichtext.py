...
def license():
    print """
    Python Fu script for Rich Text Editing in GIMP
    pyrichtext.py V 0.3 Oct, 20036
...
class RichText (object):
    ...
    def do_it (self, *a):
        ...
        for text, dict_tag in text_tuples :
            ...
            if not dict_tag.has_key("font"):
                ...
                if dict_tag.has_key("ratio"):
                    ratio = dict_tag["ratio"]
                    if  ratio > 1:
                        size = size * ratio * (1 - ratio / 15)
                    else:
                        size = size * ratio * (1 + 2 * (1 - ratio))
        ...
        for text, dict_tag in text_tuples :
            ...
            base_line  = y + line_heights[at_line]

            if ratio != 1:
                if ratio > 1:
                    real_size = size * ratio
                else:
                    real_size = size * 1 / ratio
            else:
                real_size = size
            ...
            layer = pdb.gimp_text_fontname (self.img, None,
                        ...
                        text, 0, True,
                        #size, POINTS,
                        real_size / 72.0 * self.img.resolution[0],
                        PIXELS,
                        font )
            if not layer:
                continue
            if ratio != 1:
                l_offsets = layer.offsets
                if ratio > 1:
                    nh = layer.height
                    nw = int (layer.width * 1 / ratio)
                else:
                    nh = int (layer.height * ratio)
                    nw = layer.width
                layer.scale (nw, nh, INTERPOLATION_CUBIC)
                layer.set_offsets (*l_offsets)

            x += layer.width
            pdb.plug_in_autocrop_layer (self.img, layer)
            off_x, off_y = layer.offsets
            off_y = int (base_line - layer.height)
            layer.set_offsets (off_x, off_y)
            layers.append (layer)
            layers_in_this_line.append (layer)
            

        if layers_in_this_line:
            layer_lines.append (layers_in_this_line)
        if self.justify in  (gtk.JUSTIFY_CENTER, gtk.JUSTIFY_RIGHT):
            total_width = 0
            for layers_in_this_line in layer_lines:
                line_width = layers_in_this_line[-1].width + layers_in_this_line[-1].offsets[0] - layers_in_this_line[0].offsets[0]
                total_width = max (total_width, line_width)
            print total_width
                
            for layers_in_this_line in layer_lines:
                line_width = layers_in_this_line[-1].width + layers_in_this_line[-1].offsets[0] - layers_in_this_line[0].offsets[0]
                offset_x = total_width - line_width
                if self.justify == gtk.JUSTIFY_CENTER:
                    offset_x /= 2
                if offset_x:
                    for layer in layers_in_this_line:
                        l_x, l_y = layer.offsets
                        layer.set_offsets (int(l_x + offset_x) , l_y)
    ...
    def create_window(self):
        ...
        self.ratio_adj.connect ("value-changed", self.font_changed)
...
register(
        ...
        """Heh! That is it! Version 0.3 alpha
        """,
